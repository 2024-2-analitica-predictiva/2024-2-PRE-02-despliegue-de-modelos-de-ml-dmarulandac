...
def index():
    ...
    if request.method == "POST":
        ...
        df = pd.DataFrame.from_dict(user_values, orient="index").T #dataframe unsa sola fila con los datos

        # importante, abre el pickle, carga el modelo
        with open("homework\house_predictor.pkl", "rb") as file:
            loaded_model = pickle.load(file)

        prediction = round(loaded_model.predict(df)[0][0], 2) #lo redondeo a 2 cifras
